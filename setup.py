    ...
    def build_extension(self, _ext: Extension):
        self._cmake(
            [
                ...
                # Build a single DSO that combines the core library with
                # Python bindings.
                "-DBUILD_SHARED_LIBS=OFF",
                # Install to root of install tree rather than
                # `lib/.../site-packages`, since that is what setuptools
                # expects.
                "-DOPENASSETIO_PYTHON_SITEDIR=.",
                # TODO(DF): This is required if running from a
                # venv in order to use the external (conan)
                # Python interpreter/headers. Probably not what
                # we want for deployment.
