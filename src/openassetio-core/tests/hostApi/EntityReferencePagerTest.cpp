...
SCENARIO("Using an EntityReferencePager") {
  ...
  GIVEN("a configured EntityReferencePager") {
    ...
    const std::shared_ptr<MockEntityReferencePagerInterface> mockEntityReferencePagerInterface =
        std::make_shared<MockEntityReferencePagerInterface>();

    const openassetio::hostApi::EntityReferencePager::Ptr pager =
        openassetio::hostApi::EntityReferencePager::make(mockEntityReferencePagerInterface,
                                                         fixture.hostSession);

    AND_GIVEN("plugin's hasMore expects to be called and returns false") {
      REQUIRE_CALL(*mockEntityReferencePagerInterface, hasNext(fixture.hostSession)).RETURN(false);
      WHEN("host wrapper's hasMore is called") {
        const bool hasMoreReturn = pager->hasNext();
        THEN("the value is false") { CHECK(hasMoreReturn == false); }
      }
    }
    AND_GIVEN("plugin's hasMore expects to be called and returns true") {
    ...
    AND_GIVEN("plugin's next expects to be called") {
      REQUIRE_CALL(*mockEntityReferencePagerInterface, next(fixture.hostSession));
      WHEN("host wrapper's next is called") {
        pager->next();
        THEN("pagerInterface next was called") {}
      }
    }
    AND_GIVEN("plugin's get expects to be called and returns some entity references") {
      ...
      REQUIRE_CALL(*mockEntityReferencePagerInterface, get(fixture.hostSession))
          .RETURN(testEntRefs);
      WHEN("host wrapper's get is called") {
        const openassetio::EntityReferences getReturn = pager->get();
        THEN("the returned entity references match those provided by the plugin") {
          CHECK(getReturn == testEntRefs);
        }
      }
    }
  }
