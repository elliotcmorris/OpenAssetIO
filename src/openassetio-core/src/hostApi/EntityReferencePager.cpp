...
typename EntityReferencePager::Ptr EntityReferencePager::make(
  ...
  // Trick to give private constructor access to std::make_shared.
  struct Constructible : EntityReferencePager {
    Constructible(managerApi::EntityReferencePagerInterfacePtr pagerInterface,
                  managerApi::HostSessionPtr hostSession)
        : EntityReferencePager(std::move(pagerInterface), std::move(hostSession)) {}
  };

  return std::make_shared<Constructible>(std::move(pagerInterface), std::move(hostSession));
}
