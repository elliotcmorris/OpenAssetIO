...
class OPENASSETIO_CORE_EXPORT Manager {
 public:
   ...
   * that initiated the call to `getWithRelationship`. To access the
   * data, retrieve the @ref EntityReferencePager from the callback, and
   * use its interface to traverse pages.
   *
   * @param errorCallback Callback that will be called for each failed
   * relationship query. It will be given the corresponding index of the
   * entity reference in @p entityReferences along with a populated
   ...
   * will be called on the same thread that initiated the call to
   * `getWithRelationship`. To access the data, retrieve the
